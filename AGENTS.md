...
- Computer Use screenshots now record a main-process coordinate context for the captured display (`electron/tools/computer-use/coordinates.ts`), including rendered screenshot size, native capture size, display bounds, and DPI scale factor. Follow-up click, scroll, and cursor-move actions continue to accept latest-screenshot pixel coordinates from the model, but main maps them into desktop coordinates before invoking `nut-js` and before drawing the spotlight overlay.
