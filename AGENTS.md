...
- Persistence is split:
  ...
  - **Scheduled task definitions, web lookout snapshots, reminder logs, and run history** live in the main process under `app.getPath('userData')` in `scheduled-tasks.json`; legacy `web-monitors.json` data is migrated on read. The scheduled-task runtime schedules enabled reminders/lookouts while ZuraAI is open, performs due checks once on next startup, fetches public or local-loopback http/https lookout pages directly from main, asks the renderer to summarize detected changes through the existing provider runtime, and emits OS-level Electron notifications for due reminders and changed lookouts when desktop notifications are supported.
