...
When semantic background automation cannot complete a foreground-only step, Agent Mode explicitly releases its reservation and requests the immediate targeted `computer_screenshot` with `reserve_background: false`. Main accepts that opt-out only with the short-lived, one-use permit created by a successful release in the same sender/run. This keeps the fresh screenshot bound to the exact main-issued HWND without silently reattaching the guard before the separately approved physical action.
...
Windows agent runs may reserve one external window via `background_window_attach`. Main owns identity and scoping. A targeted window `computer_screenshot` automatically reserves its returned main-issued exact HWND for an Agent run; full-screen screenshots never infer a target. An approved Agent Mode `window_focus` request with an exact HWND is also converted into that background reservation without changing focus, while title-only focus requests fail closed. Physical Computer Use needs a fresh main-issued screenshot ID. An approved targeted left click first maps its point to a meaningful enabled/visible exact-HWND UIA/MSAA element with a provider-advertised Invoke, SelectionItem/Toggle, or default action. Unnamed generic containers are rejected. Provider activation remains background-safe and keeps the guard attached, carries `semanticOutcome: unverified`, and the same run cannot dispatch the identical provider-element action again without a different verified step. While a reservation exists, an unsupported point returns `foreground_required` without releasing the guard or sending physical input. Accessibility state includes provider-advertised accelerator/access keys so shortcut outcomes can be resolved to semantic background actions without injecting keys. While a background reservation exists, main rejects `computer_key`, `computer_type`, `computer_scroll`, and `computer_cursor_position`; the agent must use target-scoped UI Automation or report `foreground_required`. Outside a background reservation, keyboard and text actions require a window-targeted screenshot and are automatically bound to its main-issued exact HWND; main briefly foregrounds and verifies that window before input, then restores the user's previous foreground window only if the target retained focus, so a concurrent user focus change is not overwritten. Whole-screen keyboard input is rejected, and the model cannot supply or replace the HWND. Delivery mode is evidence of dispatch, not semantic task completion.
...
ChatGPT Codex is a main-only OAuth exception: fixed public client, loopback callback, encrypted tokens, and fixed ChatGPT endpoints. ZuraAI may send the current narrow tool inventory through the Responses API and normalizes returned native function calls into the existing renderer-owned tool loop; execution still uses the standard main-issued approvals and tool validation. Image inputs remain disabled on this path. Local/personal use; not a broad ChatGPT proxy.
...
- Electron distributions use electron-builder's platform/architecture-aware downloads; do not point global `electronDist` at the host installation when producing universal packages.
- Package commands disable electron-builder's implicit publishing; the release workflow's publish job exclusively creates the GitHub release after both platform artifacts pass verification.
- Windows: NSIS installer + portable; user-selected install directory is final
- macOS: universal Intel/Apple Silicon DMG/zip; signing/notarization is a separate ops step (credentials in env only)
