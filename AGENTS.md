...
- OpenCode Go is a renderer-side OpenAI-compatible provider: `opencodeGoApiKey` is hydrated from main-process secure storage, `opencodeModels` lives in sanitized renderer settings, catalog import calls `https://opencode.ai/zen/go/v1/models`, and chat/title/memory requests dispatch through the shared provider runtime to `https://opencode.ai/zen/go/v1/chat/completions` with no main-process IPC channel added.
...
- OpenCode Go: `src/services/opencode.ts` (OpenAI-compatible at `https://opencode.ai/zen/go/v1`; `opencodeGoApiKey` in main-process secure storage; `opencodeModels` in sanitized renderer settings; catalog import via `/v1/models`; streaming + tool calling through the shared provider runtime)
