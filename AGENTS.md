...
- System prompt injection: `src/prompts/buildMemoryBlock.ts` builds a "## Saved Memories (Model Set Context)" block with dated bullets, oldest-first, capped at `MEMORY_BLOCK_TOKEN_BUDGET≈2000` tokens (truncates oldest first with a console warning). When the Memory skill is enabled, the block additionally appends an explicit "when to save" instruction (sourced from `settings.memoryPrompt`, defaulting to `src/prompts/defaultMemoryPrompt.ts`) so the model proactively calls `save_memory` whenever the user mentions a durable fact (age, school, preferences, projects, etc.). The instruction also renders on its own (with a `(no saved memories yet)` placeholder) so the nudge fires from the very first chat. `loadMemoryBlock(settings, scope?)` is the gated async helper used by `useStreamingChat` at both send and regenerate call sites; injection is appended after the skills section in `getEffectiveSystemPrompt`.
- Memory is shipped as a built-in skill (`skills.memory`, default enabled). The skill toggle in Settings → Skills and the mirrored toggle in Settings → Personalization → Memory share the same source of truth; legacy `memoryEnabled` / `autoMemoryEnabled` settings are migrated into the skill on first read by `migrateSkillsFromLegacySettings`.
- Model-callable tools (renderer-only, gated by `isSkillEnabled(skills, 'memory')` in `useToolCalling.getEnabledToolsForProvider`): `save_memory`, `update_memory`, `delete_memory`, `search_memories`. Defined in `src/tools/memoryTools.ts`; routed by `src/tools/executor.ts` ahead of the MCP / built-in main IPC paths. Each successful mutation fires a sonner toast and returns a `MemoryToolEvent` payload on `ToolResult.data` so the inline `MemoryUpdatePill` (see below) can render diffs without re-fetching.
- Settings UI: Settings → Personalization → Memory (`src/components/Settings/sections/MemorySection.tsx`). Lists all memories with content + timestamps + source badge (You/AI), supports inline add/edit/delete, "Clear all" with confirmation, and a single skill-driven Enable toggle (the same `skills.memory.enabled` value the Skills section toggles). The panel writes through `window.memory.*` directly — no draft mode — and live-syncs via `memory-store:changed`.
- Memory autosave instruction is user-editable as a dedicated "Memory Prompt" card in Settings → System Prompt (alongside Web Search / Code Execution / Computer Use / Chart Generation prompts), backed by `settings.memoryPrompt` (defaults to `defaultMemoryPrompt`). Empty/whitespace overrides fall back to the default instruction so the autosave nudge is never silently dropped.
