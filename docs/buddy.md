...
- Phase 1 interaction is a normal compact overlay panel rather than a mascot or edge-peek surface.
- Phase 1 entry points are overlay open/toggle controls plus a dedicated global shortcut.
...
Ship a reliable, optional Buddy Overlay that opens as a normal desktop overlay window and lets users talk to ZuraAI without opening the full dashboard.
...
3. The Buddy window is always-on-top, skip-taskbar, frameless, and positioned as a normal overlay against the active display work area.
4. Phase 1 uses explicit open/toggle controls plus a global hotkey; there is no tray-only, hover-only, or browser-extension-first flow.
...
- overlay positioning and repositioning
- cleanup on close and on app shutdown

The window implementation should:

- use Electron `screen` APIs to position against the active display `workArea`
...
- Compact: normal overlay panel shown in its default small state
- Expanded: overlay grows into a larger compact chat panel
...
- Opening the Buddy shows the compact overlay.
- Clicking expand grows the panel into its larger chat state.
- Clicking close hides the overlay instead of destroying the feature, unless the user has disabled the overlay.
...
- Use `workArea` positioning instead of raw screen bounds so the Buddy stays clear of the Windows taskbar.
...
- mascot or character-driven edge-peek behavior
- freeform draggable overlay behavior with arbitrary edge positions
...
- the Buddy opens from explicit overlay controls and global hotkey
...
- the overlay stays correctly positioned within the active display work area after display metric changes
...
- overlay positioning on single-display and multi-display setups
