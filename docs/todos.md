...
- [x] Consolidate Provider Hub connectivity state transitions and probe logic into shared helpers.
- [x] Consolidate repeated IPC registration/unregistration boilerplate where it reduces complexity.
...
- [x] Continue migrating type-only imports from context modules to shared type modules.
...
- [x] Re-run `knip` and triage remaining uncertain candidates (`playwright`, barrel-path test references).
...
- [x] Re-run `madge --circular` across `src/**` and `electron/**` and verify zero cycles.
- [x] Add a lightweight import-boundary guard to prevent manager/context back-edges from returning.
...
- [x] Replace broad IPC bridge signatures with channel-mapped typings.
- [x] Remove remaining unsafe casts in streaming/provider orchestration and parser boundaries.
- [x] Keep `unknown` only at trusted boundaries with explicit narrowing/guards.
...
- [x] Continue auditing and narrowing silent catches/fallbacks in broader renderer/main flow.
...
## Verification Pass (Phase 2)
- [x] `bun run typecheck`
- [x] `bun run test -- src/contexts/importBoundaries.test.ts src/contexts/ChatSessionManager.test.ts src/contexts/ChatSessionManager.property.test.ts src/contexts/ChatHistoryContext.actions.test.tsx`
- [x] `bun run test -- electron/preload.test.ts src/contexts/importBoundaries.test.ts`
- [x] `bun run knip`
- [x] `npx madge --circular --extensions ts,tsx src electron`
- [x] `bun run test` (suite passes; observed one intermittent timeout in `electron/mcp/mcpConnection.test.ts` during an earlier run, then stable on rerun)
...
- [x] **Phase 2: structural quality improvements**: deeper type consolidation, runtime/IPC contract tightening, and larger refactors.
