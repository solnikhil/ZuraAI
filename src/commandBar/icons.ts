...
import {
  ...
  ChartNoAxesCombined, Cloud, Box, Command, FlaskConical, FileText, Globe, Send, Wrench
...
export function getSuggestionIcon(suggestion: CommandBarSuggestion): { Icon: React.ComponentType<{ size?: number; className?: string }>, iconClass: string } {
  ...
  if (suggestion.id === 'open-performance') return { Icon: ChartNoAxesCombined, iconClass: 'app-titlebar__commandbar-item-icon--navigate' }
