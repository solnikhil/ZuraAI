...
export function getSuggestionIcon(suggestion: CommandBarSuggestion): {
  Icon: React.ComponentType<{ size?: number; className?: string }>
  iconClass: string
} {
  // Navigation actions
  if (suggestion.id === 'go-settings')
    return { Icon: SettingsIcon, iconClass: 'app-titlebar__commandbar-item-icon--navigate' }
  if (suggestion.id === 'go-chat')
    return { Icon: LayoutDashboard, iconClass: 'app-titlebar__commandbar-item-icon--navigate' }

  // Settings section actions
  if (suggestion.id === 'go-settings-usage')
    return { Icon: ChartNoAxesCombined, iconClass: 'app-titlebar__commandbar-item-icon--navigate' }
  if (
    suggestion.id === 'go-settings-providers' ||
    suggestion.id === 'go-settings-models' ||
  ...
  if (suggestion.id === 'go-settings-extensions')
    return { Icon: Wrench, iconClass: 'app-titlebar__commandbar-item-icon--navigate' }
  if (suggestion.id === 'go-settings-search-apis')
  ...
  if (suggestion.id === 'new-chat')
    return { Icon: Plus, iconClass: 'app-titlebar__commandbar-item-icon--create' }
  if (suggestion.id === 'copy-chat-debug-id')
    return { Icon: Command, iconClass: 'app-titlebar__commandbar-item-icon--navigate' }
  if (suggestion.id === 'open-chat-debug-panel')
    return { Icon: Terminal, iconClass: 'app-titlebar__commandbar-item-icon--navigate' }
  if (suggestion.id === 'toggle-memory-monitor')
    return { Icon: ChartNoAxesCombined, iconClass: 'app-titlebar__commandbar-item-icon--toggle' }
