...
const DEEPSEEK_BASE_URL = getProviderEndpoint('deepseek', 'baseUrl') ?? 'https://api.deepseek.com'
...
): DeepSeekWireReasoningEffort | undefined {
...
export function convertToDeepSeekMessages(messages: ChatMessage[]): DeepSeekMessage[] {
  return messages.map((msg) => {
    ...
    const toolCalls = (msg as ChatMessage & { tool_calls?: DeepSeekMessage['tool_calls'] })
      .tool_calls
    if (toolCalls) {
...
export function mapDeepSeekModelToConfiguredModel(
  model: DeepSeekModel
