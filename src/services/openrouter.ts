  ...
  return messages.map((message, index) => {
    const rawContent =
      typeof message.content === 'string'
        ...
        : Array.isArray(message.content)
          ? message.content
              .map((part) =>
                'text' in part && typeof part.text === 'string' ? part.text : '[non-text]'
              )
              .join(' ')
          : ''
    ...
    return {
      ...
      hasToolCalls: Array.isArray(
        (message as ChatMessage & { tool_calls?: ServiceToolCall[] }).tool_calls
      )
        ? ((message as ChatMessage & { tool_calls?: ServiceToolCall[] }).tool_calls?.length || 0) >
          0
        : false,
    }
  })
