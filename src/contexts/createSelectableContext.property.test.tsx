...
describe('Context Selector Property Tests', () => {
    ...
    /**
     * Feature: electron-performance-optimization, Property 20: Context Selector Re-render Prevention
     * 
     * *For any* settings context update via dispatch, only components that use the specific changed
     * value (via selector) SHALL re-render.
     * 
        ...
        it('should only re-render components when their selected value changes via dispatch', async () => {
            await fc.assert(
                fc.asyncProperty(
                    ...
                    fc.constantFrom('theme', 'fontSize', 'apiKey') as fc.Arbitrary<'theme' | 'fontSize' | 'apiKey'>,
                            ...
                            return (
                                <>
                                    ...
                                    <button 
                                        ...
                                        onClick={() => dispatch(prev => ({ 
                                            ...prev, 
                                            modelConfig: { ...prev.modelConfig, temperature: prev.modelConfig.temperature + 0.1 }
                                        }))}
                                    >
                                        New Temperature
                                    </button>
    ...
    /**
     * Feature: electron-performance-optimization, Property 29: Chat History Selector Pattern
     * 
     * *For any* component using ChatHistoryContext with a selector, it SHALL only
        ...
        it('should only re-render session list component when sessions change via dispatch', async () => {
            await fc.assert(
                fc.asyncProperty(
                    ...
                    fc.constantFrom('sessions', 'currentSessionId', 'isLoading') as fc.Arbitrary<'sessions' | 'currentSessionId' | 'isLoading'>,
                    async (initialState, fieldToChange) => {
                ...
                fc.asyncProperty(
                    ...
                    async (initialState) => {
                        ...
                        function Consumer() {
                            ...
                            return <div data-testid="combined">{combined.theme}-{combined.fontSize}</div>
                        }
            ...
            fc.asyncProperty(
                ...
                async (initialState) => {
                    ...
                    const Consumer = memo(function Consumer() {
                        ...
                        const fontSize = useSelector(state => state.fontSize)
                        renderCount.current++
                        return <div data-testid="values">{theme}-{fontSize}</div>
                    })
            ...
            fc.asyncProperty(
                ...
                async (initialState) => {
                    ...
                    const Consumer = memo(function Consumer() {
                        ...
                        const apiKey = useSelector(state => state.apiKey)
                        renderCount.current++
                        return <div data-testid="values">{theme}-{apiKey.length}</div>
                    })
...
// ============================================================================
// Property 30: Context Provider Memoization
// ============================================================================
...
// ============================================================================
// Property 31: Lazy PDF Context Loading
// ============================================================================
 ...
 * Property 31: Lazy PDF Context Loading
 * 
 * *For any* route other than `/pdf-chat`, the PDFDocumentContext SHALL not be
 * initialized or loaded.
 * 
 * **Validates: Requirements 8.4**
 * 
 * This property verifies that the PDFDocumentContext is lazily loaded only when
 * the /pdf-chat route is accessed. The implementation uses:
 * 1. React.lazy() for the LazyPDFChatWrapper component
 * 2. PDFDocumentProvider is only rendered inside LazyPDFChatWrapper
 * 3. usePDFDocuments() returns default empty values when outside the provider
 * 
 * Note: These tests verify the lazy loading behavior by checking that:
 * - The usePDFDocuments hook returns default values outside the provider
 * - The PDFDocumentProvider is only instantiated for the /pdf-chat route
 */
describe('Property 31: Lazy PDF Context Loading', () => {
    afterEach(() => {
        cleanup()
    })

    /**
     * Test that usePDFDocuments returns default values when used outside PDFDocumentProvider.
     * This enables components to safely call the hook without errors on non-PDF routes.
     */
    it('should return default empty values when PDFDocumentContext is not provided', async () => {
        // Import the actual hook to test its behavior outside provider
        const { usePDFDocuments } = await import('./PDFDocumentContext')
        
        await fc.assert(
            fc.asyncProperty(
                fc.constant(null), // No input needed, just testing default behavior
                async () => {
                    let contextValue: ReturnType<typeof usePDFDocuments> | null = null
                    
                    // Component that uses the hook outside of PDFDocumentProvider
                    function TestComponent() {
                        contextValue = usePDFDocuments()
                        return <div data-testid="test">Test</div>
                    }
                    
                    // Render without PDFDocumentProvider (simulating non-PDF route)
                    render(<TestComponent />)
                    
                    // Property: Hook should return default empty values, not throw
                    expect(contextValue).not.toBeNull()
                    expect(contextValue!.loadedDocumentsMap.size).toBe(0)
                    expect(contextValue!.loadedDocuments).toEqual([])
                    expect(contextValue!.activeDocumentId).toBeNull()
                    expect(contextValue!.currentPage).toBe(1)
                    expect(contextValue!.activeDocumentPageCount).toBe(0)
                    
                    // Property: Setter functions should be no-ops (not throw)
                    expect(() => contextValue!.setLoadedDocuments(new Map())).not.toThrow()
                    expect(() => contextValue!.setActiveDocumentId('test')).not.toThrow()
                    expect(() => contextValue!.setCurrentPage(5)).not.toThrow()
    ...
    /**
     * Test that PDFDocumentProvider properly initializes context when used.
     * This verifies the provider works correctly when the /pdf-chat route is accessed.
     */
    it('should properly initialize context when PDFDocumentProvider is used', async () => {
        const { PDFDocumentProvider, usePDFDocuments } = await import('./PDFDocumentContext')
        ...
        await fc.assert(
            fc.asyncProperty(
                ...
                async () => {
                    ...
                    // Render WITH PDFDocumentProvider (simulating /pdf-chat route)
                    render(
                        <PDFDocumentProvider>
                            <TestComponent />
                        </PDFDocumentProvider>
                    ...
                    expect(contextValue).not.toBeNull()
                    expect(contextValue!.loadedDocumentsMap).toBeInstanceOf(Map)
                    expect(Array.isArray(contextValue!.loadedDocuments)).toBe(true)
                    expect(typeof contextValue!.setLoadedDocuments).toBe('function')
                    expect(typeof contextValue!.setActiveDocumentId).toBe('function')
                    expect(typeof contextValue!.setCurrentPage).toBe('function')
    ...
    /**
     * Test that usePDFDocumentsStrict throws when used outside PDFDocumentProvider.
     * This verifies the strict hook enforces proper context usage.
     */
    it('should throw error when usePDFDocumentsStrict is used outside provider', async () => {
        const { usePDFDocumentsStrict } = await import('./PDFDocumentContext')
        
        await fc.assert(
            fc.asyncProperty(
                fc.constant(null),
                async () => {
                    // Component that uses the strict hook outside of PDFDocumentProvider
                    function TestComponent() {
                        usePDFDocumentsStrict()
                        return <div>Test</div>
                    }
                    
                    // Property: Strict hook should throw when used outside provider
                    // This ensures components that REQUIRE the context fail fast
                    expect(() => {
                        render(<TestComponent />)
                    }).toThrow('usePDFDocumentsStrict must be used within PDFDocumentProvider')
    ...
    /**
     * Test that the lazy loading pattern allows components to safely check for PDF context.
     * This simulates how the Sidebar component can check for loaded documents without errors.
     */
    it('should allow safe context checking on non-PDF routes', async () => {
        const { usePDFDocuments } = await import('./PDFDocumentContext')
        ...
        await fc.assert(
            fc.asyncProperty(
                ...
                async (documentNames) => {
                    ...
                    // Simulates a sidebar component that checks for loaded PDFs
                    function SidebarComponent() {
                        const { loadedDocuments } = usePDFDocuments()
                        hasDocuments = loadedDocuments.length > 0
                        documentCount = loadedDocuments.length
                        return (
                            <div data-testid="sidebar">
                                {hasDocuments ? `${documentCount} PDFs` : 'No PDFs'}
                            </div>
                        )
                    }
                    
                    // Render outside PDFDocumentProvider (non-PDF route)
                    render(<SidebarComponent />)
                    
                    // Property: Component should render without errors
                    expect(screen.getByTestId('sidebar')).toBeDefined()
                    
                    // Property: Should show no documents (default empty state)
                    expect(hasDocuments).toBe(false)
                    expect(documentCount).toBe(0)
                    expect(screen.getByTestId('sidebar').textContent).toBe('No PDFs')
    ...
    /**
     * Test that context state updates work correctly when provider is present.
     * This verifies the full functionality when on the /pdf-chat route.
     */
    it('should allow state updates when PDFDocumentProvider is present', async () => {
        const { PDFDocumentProvider, usePDFDocuments } = await import('./PDFDocumentContext')
            ...
            fc.asyncProperty(
                ...
                async (documentId, documentName) => {
                    ...
                    function TestComponent() {
                        ...
                        currentActiveId = activeDocumentId
                        currentPage = page
                        ...
                        return (
                            <>
                                ...
                                <div data-testid="active-id">{activeDocumentId || 'none'}</div>
                    ...
                    // Property: State should update when provider is present
                    expect(screen.getByTestId('active-id').textContent).toBe(documentId)
                    ...
                    // Property: Page state should update
