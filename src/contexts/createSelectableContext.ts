 ...
 * - Requirement 5.1: WHEN a context value changes, THE SettingsContext SHALL only 
 *   trigger re-renders in components that use the changed value
 * - Requirement 8.2: THE ChatHistoryContext SHALL implement a selector pattern to 
 ...
 * 
 * @module createSelectableContext
 */
...
export function createSelectableContext<T extends object>(): SelectableContext<T> {
    ...
    function useSelector<R>(
        ...
        equalityFn: EqualityFn<R> = strictEqual
    ): R {
    ...
    function useSubscribe(callback: (state: T) => void): void {
        ...
        if (context === null) {
            throw new Error(
                'useSubscribe must be used within a SelectableContext Provider'
            )
        }
