/**
 * Keeps in-flight assistant output separate from persisted chat history.
 */
...
export function StreamingProvider({ children }: { children: React.ReactNode }) {
  ...
  const [streamingState, setStreamingState] =
    useState<StreamingMessageState>(INITIAL_STREAMING_STATE)
  ...
  const isMessageStreaming = useCallback((sessionId: string, messageId: string): boolean => {
    ...
    return current.isStreaming && current.sessionId === sessionId && current.messageId === messageId
  }, [])
  ...
  const getStreamingContent = useCallback(
    (sessionId: string, messageId: string): StreamingMessageState | null => {
      const current = stateRef.current
      if (
        current.isStreaming &&
