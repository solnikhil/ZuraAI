...
import { Check, ChevronDown, Cloud, Code, Clock, Plus, RefreshCcw, User } from 'lucide-react'
...
export default function GitHubWorkspace() {
  ...
  if (state.account.status !== 'signed_in') {
    const signingIn = state.account.status === 'signing_in'
    const setupMessage = state.account.status === 'signed_out' ? state.account.error : undefined
    return (
      <section className="github-workspace github-workspace--auth" aria-label="GitHub sign in">
        <div className="github-workspace-auth">
          ...
          {signingIn ? (
            ...
            <button className="github-workspace-auth__continue" type="button" autoFocus onClick={() => void window.githubWorkspace.startSignIn().then((account) => setState({ ...state, account }))}>
              <span>Continue with GitHub</span><kbd>Enter</kbd>
            </button>
          )}
        ...
        <style>{`
          ...
          .github-workspace-auth__continue { min-width: 220px; height: 40px; display: flex; align-items: center; justify-content: space-between; gap: 18px; margin-top: 7px; padding: 0 9px 0 15px; border: 1px solid rgba(255,245,241,.16); border-radius: 8px; background: rgba(255,245,241,.88); color: rgba(34,29,30,.94); font: inherit; font-size: 12px; font-weight: 700; cursor: pointer; }
          .github-workspace-auth__continue:hover, .github-workspace-auth__continue:focus-visible { border-color: rgba(201,146,131,.7); background: rgba(255,247,244,.96); outline: none; }
          .github-workspace-auth__continue kbd { padding: 3px 5px; border: 1px solid rgba(39,32,33,.14); border-radius: 4px; background: rgba(39,32,33,.07); color: rgba(39,32,33,.62); font: inherit; font-size: 8px; font-weight: 700; }
  ...
  return (
    <section className="github-workspace" aria-label="GitHub Workspace">
      <header className="github-workspace__header">
        <div className="github-workspace__identity"><Code size={17} /><strong>GitHub Workspace</strong></div>
        {state.account.status === 'signed_in' ? (
          ...
          <button type="button" onClick={() => void window.githubWorkspace.startSignIn().then((account) => setState({ ...state, account }))}><User size={14} /> Sign in</button>
        )}
      </header>
      ...
      <footer className="github-workspace__footer">
        <div className="github-workspace__mark"><Code size={19} /></div>
      ...
      <style>{`
        ...
        .github-workspace__header, .github-workspace__footer { flex: none; display: flex; align-items: center; border-color: rgba(255,239,232,.1); }
        .github-workspace__header { height: 38px; justify-content: space-between; padding: 0 12px; border-bottom: 1px solid rgba(255,239,232,.1); }
        .github-workspace__identity, .github-workspace__account { display: flex; align-items: center; gap: 8px; font-size: 12px; }
        ...
        .github-workspace__mark { width: 25px; display: grid; place-items: center; color: rgba(255,244,240,.72); }
