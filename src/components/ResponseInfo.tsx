  ...
  const renderTokenBarRow = (label: string, value: number, opacity: number) => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontSize: '10.5px', color: 'var(--theme-text-muted)', fontWeight: 500 }}>
          {label}
        </span>
        <span
          style={{
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          ...
          {finishReason && (
            ...
            >
              ...
              <span
                style={{ fontSize: '13px', fontWeight: 600, color: 'var(--theme-text-primary)' }}
              >
                {finishReason}
              </span>
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            {shouldShowTokenDetails && (
              ...
              >
                ...
                {cachedInputTokens > 0 &&
                  renderTokenBarRow('Cache hit input', cachedInputTokens, 0.55)}
                {cacheMissInputTokens > 0 &&
                  renderTokenBarRow('Cache miss input', cacheMissInputTokens, 0.4)}
                {cacheWriteInputTokens > 0 &&
                  renderTokenBarRow('Cache write input', cacheWriteInputTokens, 0.48)}
                {cachedOutputTokens > 0 &&
                  renderTokenBarRow('Cached output', cachedOutputTokens, 0.55)}
              </div>
            )}
          </div>
            ...
            <div style={{ display: 'flex', gap: '8px', marginTop: '2px' }}>
              {usage.tps !== undefined && (
                ...
                >
                  ...
                  >
                    {fmtTps(usage.tps)}{' '}
                    <span style={{ fontSize: '10px', fontWeight: 500, opacity: 0.7 }}>t/s</span>
                  </div>
                </div>
              )}
              {usage.ttft !== undefined && (
                ...
                >
                  ...
                  >
                    {fmt(usage.ttft)}
                    <span style={{ fontSize: '10px', fontWeight: 500, opacity: 0.6 }}>ms</span>
                  </div>
                </div>
              )}
            </div>
