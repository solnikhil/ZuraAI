...
function mergeUsage(
  ...
  return {
    ...
    cachedInputTokens:
      (existing.cachedInputTokens || 0) + (followUp.prompt_cache_tokens || 0) || undefined,
    cachedOutputTokens:
      (existing.cachedOutputTokens || 0) + (followUp.completion_cache_tokens || 0) || undefined,
  }
}
...
export function useOpenRouterStreaming({
  ...
  const streamOpenRouter = useCallback(
    async (options: OpenRouterStreamingOptions): Promise<StreamingResult> => {
      const {
        sessionId,
        messageId,
