/**
 * useFireworksStreaming - Provider-specific streaming hook for Fireworks AI
 *
 * Extracts Fireworks streaming logic from useStreamingChat to reduce complexity.
...
export interface UseFireworksStreamingOptions {
...
export interface UseFireworksStreamingReturn {
  streamFireworks: (options: ToolCallingOptions) => Promise<StreamingResult>
}
...
/**
 * Hook for Fireworks-specific streaming logic with tool calling support
 */
export function useFireworksStreaming({
  settings,
  toolCalling,
  updateStreamingMessage,
  flushThrottledUpdates,
  throttledUpdateStreamingMessage,
}: UseFireworksStreamingOptions): UseFireworksStreamingReturn {
  const { updateStreaming } = useStreamingActions()
  const updateInterval = getStreamingUpdateInterval()

  const streamFireworks = useCallback(
    async (options: ToolCallingOptions): Promise<StreamingResult> => {
      ...
      const modelSupportsTools = !MODELS_WITHOUT_TOOL_SUPPORT.includes(settings.aiModel)
      const tools = canUseTools && modelSupportsTools ? getToolsForRequest() : null
      const fireworksTools = tools && Array.isArray(tools) ? tools : undefined
        ...
        if (toolResult.needsFollowUp && toolResult.formattedResults.length > 0) {
          ...
          while (hasMoreToolCalls && researchRound < SAFETY_CAP) {
            ...
            for await (const chunk of streamFireworksCompletion(
              ...
              settings.aiModel,
              followUpMessages,
              { temperature: settings.temperature, tools: fireworksTools, toolChoice: undefined, signal }
            )) {
              const delta = chunk.choices?.[0]?.delta?.content || ''
      ...
      updateStreamingMessage(sessionId, messageId, {
        ...
        model: `fireworks/${settings.aiModel}`,
