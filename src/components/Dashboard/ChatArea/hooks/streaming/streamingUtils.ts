...
export const FINAL_SYNTHESIS_PROMPT =
  '\n\n*** FINAL SYNTHESIS REQUIRED *** You have enough search results to produce the best supported answer from the gathered evidence. Do not call any more tools or web_search. Provide your final synthesized answer now using only the results already returned. If the results are inconclusive, say that clearly, summarize the strongest relevant evidence, and state what could not be verified instead of searching again. For model/provider release, availability, capability, and pricing claims, require official vendor docs, blog posts, pricing pages, or release notes before presenting the claim as verified. Never return an empty response.\n\n'
export const FINAL_SYNTHESIS_BUDGET_EXHAUSTED_PROMPT =
  '\n\n*** FINAL SYNTHESIS REQUIRED: WEB SEARCH BUDGET EXHAUSTED *** The available web_search budget for this response has been used. Do not call any more tools or web_search. Produce the best supported answer from the gathered evidence using only the results already returned. If the gathered evidence is incomplete or conflicting, say so clearly, summarize the strongest relevant evidence, and state what could not be verified instead of searching again. For model/provider release, availability, capability, and pricing claims, require official vendor docs, blog posts, pricing pages, or release notes before presenting the claim as verified. Never return an empty response.\n\n'
export const FINAL_SYNTHESIS_EMPTY_BATCH_PROMPT =
  '\n\n*** FINAL SYNTHESIS REQUIRED: NO EXECUTABLE WEB SEARCH REMAINED *** The last attempted web_search batch did not contain an executable query. Do not call any more tools or web_search. Produce the best supported answer from the gathered evidence using only the results already returned. If the gathered evidence is incomplete or conflicting, say so clearly, summarize the strongest relevant evidence, and state what could not be verified instead of searching again. For model/provider release, availability, capability, and pricing claims, require official vendor docs, blog posts, pricing pages, or release notes before presenting the claim as verified. Never return an empty response.\n\n'
export const FINAL_SYNTHESIS_RECOVERY_PROMPT =
  '\n\n*** FINAL ANSWER REQUIRED *** Your previous synthesis attempt returned no answer. Do not call any tools or web_search. Respond with at least one concise paragraph using only the results already returned. If the evidence is inconclusive, say so directly, summarize what was checked, and state unknowns instead of searching again. For model/provider release, availability, capability, and pricing claims, require official vendor evidence before presenting the claim as verified.\n\n'
const FINAL_SYNTHESIS_PLAIN_TEXT_ONLY_PROMPT =
  '\n\n*** PLAIN TEXT ONLY FINAL ANSWER REQUIRED *** You must respond with plain assistant text only. Do not emit tool_calls, function calls, JSON, XML, markdown code fences, or any request for more searching. Do not call any tools or web_search. Write at least one concise paragraph using only the returned search results. If the evidence is inconclusive, say so directly, summarize the strongest relevant findings, and state unknowns instead of searching again. For model/provider release, availability, capability, and pricing claims, require official vendor evidence before presenting the claim as verified.\n\n'
...
export function buildThinkingBlocksFromResults(
  ...
  for (const tr of toolResults) {
    ...
    if (tr.toolCall.name === 'web_search') {
      // Allow budget-skipped web_search results to create a searching block so the
      // "budget finished" outcome is visible in the normal thinking timeline like other searches.
      // (Previously all skipped web searches were hidden here.)
      const metadata = tr.result?.metadata as Record<string, unknown> | undefined
      if (wasSkipped && metadata?.skippedReason !== 'budget') {
        continue
      }
