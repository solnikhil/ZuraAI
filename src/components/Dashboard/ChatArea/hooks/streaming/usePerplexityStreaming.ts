...
export function usePerplexityStreaming({
  ...
  const streamPerplexity = useCallback(
    async (options: ProviderStreamingOptions): Promise<StreamingResult> => {
      const { sessionId, messageId, messages: optimizedHistory, startTime, signal } = options
      ...
      const tps =
        usage.outputTokens > 0 && latency > 0 ? usage.outputTokens / (latency / 1000) : undefined
