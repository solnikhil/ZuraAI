...
import {
  ...
  type CommandCenterSkeletonTemplate,
} from '@/components/commandCenter/CommandCenterSkeletons'
...
export default function CommandCenterExtensionHost({ extensionId, commandId }: { extensionId: string; commandId: string }) {
  ...
  if (!view) {
    return (
      <section className="zura-extension-host" aria-label="Loading extension">
        <header className="zura-extension-host__header">
          <strong>Extension</strong>
        </header>
        <CommandCenterSkeleton template="list" label="Loading extension" />
  ...
  return (
    <section className="zura-extension-host" aria-label={view.title} onKeyDown={(event) => { if (event.key === 'Escape' && history.length) { event.preventDefault(); event.stopPropagation(); void goBack() } }}>
      <header className="zura-extension-host__header">
        ...
        {busy && view.kind !== 'loading' && <Loader2 size={13} className="is-spinning" aria-label="Working" />}
      </header>
      ...
      {view.kind === 'loading' && (
        <LoadingView title={view.title} description={view.description} skeleton={view.skeleton} />
      )}
...
function LoadingView({
  ...
  const template: CommandCenterSkeletonTemplate = isCommandCenterSkeletonTemplate(skeleton)
    ? skeleton
    : 'rows'
  return (
    <div className="zura-extension-loading">
      {description ? <p className="zura-extension-loading__hint">{description}</p> : null}
      <CommandCenterSkeleton template={template} label={title || 'Loading'} />
    </div>
  )
}
