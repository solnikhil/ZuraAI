...
export const artifactToolDefinitions: ToolDescriptor[] = [
  {
    ...
    description: 'Create a durable chat artifact for substantial text, markdown, code, HTML, JSON, SVG, or Mermaid content. Use this when the user will likely edit, preview, reuse, or export the output.',
    parameters: {
      type: 'object',
      properties: {
        title: { type: 'string', description: 'Short user-visible artifact title.' },
        kind: { type: 'string', description: 'Artifact content kind.', enum: ['text', 'markdown', 'code', 'html', 'json', 'svg', 'mermaid'] },
  ...
  },
  {
    ...
    description: 'Update an existing artifact by adding a new version. Use this for revisions instead of creating duplicate artifacts.',
    parameters: {
      type: 'object',
      properties: {
        ...
        content: { type: 'string', description: 'Full replacement content for the new artifact version.' },
...
export function isArtifactToolName(toolName: string): toolName is 'artifact_create' | 'artifact_update' {
...
export async function executeArtifactTool(
  ...
  if (!context.sessionId) return fail('No active chat session is available for artifact storage.', startTime)
