 ...
 * Task: 7.6 Write property tests for rendering optimization
 * 
...
const fullMessageArbitrary = fc.record({
    ...
    thinking: fc.option(fc.string({ minLength: 0, maxLength: 200 }), { nil: undefined }),
    thinkingDuration: fc.option(fc.integer({ min: 0, max: 60000 }), { nil: undefined }),
    thinkingBlocks: fc.option(fc.array(thinkingBlockArbitrary, { minLength: 0, maxLength: 5 }), { nil: undefined }),
    latency: fc.option(fc.integer({ min: 0, max: 30000 }), { nil: undefined }),
    finishReason: fc.option(fc.constantFrom('stop', 'length', 'tool_calls'), { nil: undefined }),
    requestedMaxTokens: fc.option(fc.integer({ min: 100, max: 100000 }), { nil: undefined }),
    files: fc.option(fc.array(fileAttachmentArbitrary, { minLength: 0, maxLength: 3 }), { nil: undefined }),
    toolResults: fc.option(fc.array(toolResultArbitrary, { minLength: 0, maxLength: 3 }), { nil: undefined }),
    usage: fc.option(usageArbitrary, { nil: undefined }),
    researchStatus: fc.option(researchStatusArbitrary, { nil: undefined }),
    responseVersions: fc.option(fc.array(responseVersionArbitrary, { minLength: 0, maxLength: 3 }), { nil: undefined }),
    currentVersionIndex: fc.option(fc.integer({ min: 0, max: 5 }), { nil: undefined }),
})


// ============================================================================
// Property 21: Message Component Memoization Tests
// ============================================================================
...
describe('Property 21: Message Component Memoization', () => {
    ...
    it('should return false when research status changes', () => {
        fc.assert(
            fc.property(
                fullMessageArbitrary,
                researchStatusArbitrary,
                (message, newResearchStatus) => {
        ...
        fc.assert(
            fc.property(
                ...
                (message) => {
                    ...
                    const nextProps = { message: { ...minimalMessage }, isStreaming: false, onCopy, onRegenerate }
...
// ============================================================================
// Property 20: Context Selector Re-render Prevention (Split Settings Context)
// ============================================================================
...
const testUISettingsArbitrary = fc.record({
    ...
    titleBarDensity: fc.constantFrom('comfortable', 'compact') as fc.Arbitrary<'comfortable' | 'compact'>,
...
describe('Property 20: Context Selector Re-render Prevention', () => {
            ...
            await fc.assert(
                fc.asyncProperty(
                    ...
                    async (uiSettings, configSettings) => {
                        ...
                        const { Provider: UIProvider, useSelector: useUISelector, useDispatch: useUIDispatch } = 
                ...
                fc.asyncProperty(
                    ...
                    async (uiSettings, configSettings) => {
                        const { Provider: UIProvider, useSelector: useUISelector } = 
                            createSelectableContext<TestUISettings>()
                        const { Provider: ConfigProvider, useSelector: useConfigSelector, useDispatch: useConfigDispatch } = 
                            createSelectableContext<TestConfigSettings>()
    ...
    describe('ChatHistoryContext Selector Isolation', () => {
        ...
        it('should only re-render sessions list when sessions change', async () => {
            await fc.assert(
                fc.asyncProperty(
                    testChatHistoryArbitrary,
                    async (initialState) => {
                        const { Provider, useSelector, useDispatch } = createSelectableContext<TestChatHistoryState>()
    ...
    const testChatStateArbitrary = fc.record({
        sessions: fc.array(
            fc.record({
                ...
                messages: fc.array(fc.string({ minLength: 1, maxLength: 100 }), { minLength: 0, maxLength: 10 }),
            }),
        ...
        await fc.assert(
            fc.asyncProperty(
                ...
                async (streamingState, chatState, contentUpdates) => {
                    ...
                    const { Provider: StreamingProvider, useSelector: useStreamingSelector, useDispatch: useStreamingDispatch } = 
        ...
        await fc.assert(
            fc.asyncProperty(
                ...
                async (streamingState, chatState) => {
                    const { Provider: StreamingProvider, useSelector: useStreamingSelector, useDispatch: useStreamingDispatch } = 
                        createSelectableContext<TestStreamingState>()
                    const { Provider: ChatProvider, useSelector: useChatSelector, useDispatch: useChatDispatch } = 
                        createSelectableContext<TestChatState>()
                    ...
                    const StreamingMessage = memo(function StreamingMessage() {
                        ...
                        return (
                            <div data-testid="streaming-message">
                                {isStreaming ? 'Streaming: ' : 'Done: '}{content}
                            </div>
                        )
                    })
                    ...
                    const MessageList = memo(function MessageList() {
                        ...
                        return (
                            <div data-testid="message-list">
                                {sessions.map(s => (
                                    <div key={s.id}>{s.title}: {s.messages.length} messages</div>
                                ))}
                            </div>
                        )
                    })
                        ...
                        return (
                            <>
                                <button 
                                    ...
                                    onClick={() => streamingDispatch(prev => ({ 
                                        ...prev, 
                                        isStreaming: true,
                                        content: ''
                                    }))}
                                ...
                                <button 
                                    ...
                                    onClick={() => chatDispatch(prev => {
                                        if (prev.sessions.length === 0) return prev
                                        const updated = [...prev.sessions]
                                        updated[0] = { ...updated[0], messages: [...updated[0].messages, 'new message'] }
