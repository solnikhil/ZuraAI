...
function getEnabledToolsForProviderLogic(
  ...
  let tools: string[] = enabledTools && enabledTools.length > 0 ? [...enabledTools] : allToolNames
      ...
      fc.property(
        ...
        (maxRounds, searchCountBase) => {
          ...
          const hasProgressIndicator =
            context.includes('WEB SEARCH PROGRESS') || context.includes('RESEARCH PROGRESS')
      ...
      fc.property(
        ...
        (maxRounds, offset) => {
          ...
          const hasFinalAnswerInstruction =
            context.toLowerCase().includes('final') && context.toLowerCase().includes('answer')
...
describe('Unknown Tool Validation', () => {
  ...
  it('Property: Only defined tools exist in the registry', () => {
    fc.assert(
      fc.property(fc.string({ minLength: 1, maxLength: 50 }), (randomToolName) => {
