...
const ARTIFACT_TOOLS = ['artifact_create', 'artifact_update']

const AGENT_SKILL_TOOLS = ['activate_skill']

const MCP_AGENT_TOOLS = ['mcp_request_add']
...
export function useToolCalling() {
  ...
  const availableTools = useMemo(() => getAllToolDefinitions(runtimeMcpTools), [runtimeMcpTools])
  ...
  const getEnabledToolsForProvider = (currentRuntimeMcpTools = runtimeMcpTools) => {
    ...
    const nativeWindowsAgentToolsEnabled = settings.assistantMode === 'agent' && isWindowsRuntime()
    ...
    const terminalSurfaceEnabled = isWindowsRuntime() && isSkillEnabled(settings.skills, 'terminal')
    ...
    const agentSkillsSurfaceEnabled =
      ...
      settings.agentSkills.catalog.some(
        (skill) => !settings.agentSkills.disabledSkillNames.includes(skill.name)
      )
    ...
    const computerPriority = new Map(
      COMPUTER_USE_TOOLS.map((tool, index) => [tool, computerPriorityOffset + index])
    )
  ...
  const handleToolCalls = async (
    ...
    try {
      ...
      const { results, formattedResults, executionSummary } = await processToolCalls(response, {
        ...
        requestToolApproval:
          settings.assistantMode === 'agent' ? approvalCallbacks?.requestToolApproval : undefined,
        ...
        onToolStart: (toolCall) => {
          setToolState((prev) => ({
            ...prev,
            activeToolCalls: prev.activeToolCalls.some(
              (activeToolCall) => activeToolCall.id === toolCall.id
            )
  ...
  const getResearchContext = (actualSearchCount?: number, maxRoundsOverride?: number): string => {
    const maxRounds = maxRoundsOverride ?? toolState.researchMode.maxRounds
    const isActive =
      maxRoundsOverride !== undefined ? maxRounds >= 0 : toolState.researchMode.isActive
    if (!isActive) {
      return ''
    }
  ...
  const isMcpRegistryPending =
    mcp?.isSupported === true && (mcp.isLoading || mcp.isRefreshing) && shouldEnableTools(settings)
