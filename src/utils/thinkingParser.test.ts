...
const safeContentArb = fc.string({ minLength: 1, maxLength: 200 }).filter(
  (s) =>
...
const xmlThinkingContentArb = fc
  .record({
...
describe('Thinking Content Parsing', () => {
  ...
  describe('Property 8: Thinking content is parsed correctly', () => {
    it('XML-style tags are parsed correctly', () => {
      fc.assert(
        fc.property(xmlThinkingContentArb, ({ raw, expectedThinking, expectedAnswer }) => {
    ...
    it('Markdown-style thinking markers are parsed correctly', () => {
      fc.assert(
        fc.property(markdownThinkingContentArb, ({ raw, expectedThinking, expectedAnswer }) => {
          const result = parseThinkingContent(raw)
    ...
    it('Final Answer only format is parsed correctly', () => {
      fc.assert(
        fc.property(finalAnswerOnlyArb, ({ raw, expectedThinking, expectedAnswer }) => {
          const result = parseThinkingContent(raw)
    ...
    it('Plain content without markers returns undefined thinking', () => {
      fc.assert(
        fc.property(plainContentArb, ({ raw, expectedThinking, expectedAnswer }) => {
          const result = parseThinkingContent(raw)
