...
export function buildOptimizedContextWithTrace<
  T extends { role: string; content: string; id?: string },
>(
  messages: T[],
  newUserMessage: string | T,
  systemPrompt: string | undefined,
  ...
  // Truncate if needed
  const { truncatedMessages, wasTruncated, originalTokens, finalTokens } = truncateHistory(
    fullHistory,
    systemPrompt,
    model,
    {
