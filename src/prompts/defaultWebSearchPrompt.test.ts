...
describe('defaultWebSearchPrompt', () => {
  it('defines a structured research lifecycle for deep and thorough research', () => {
    ...
    expect(defaultWebSearchPrompt).toContain('classify the evidence internally as one of: enough, missing critical source, conflicting, or off-topic')
    expect(defaultWebSearchPrompt).toContain('Continue only for a named unresolved evidence gap')
    expect(defaultWebSearchPrompt).toContain('Deep or thorough research means better facets, better sources, and better synthesis, not a high number of searches')
  })
  ...
  it('includes anti-runaway search rules', () => {
    ...
    expect(defaultWebSearchPrompt).toContain('Do not use remaining search budget as a reason to continue')
    expect(defaultWebSearchPrompt).toContain('If the next search would not change the final answer, synthesize instead')
  })
