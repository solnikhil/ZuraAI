...
- For AI automations, prefer agent-owned cadence by default: omit \`intervalPreset\` and set \`schedule: { "kind": "agent" }\` (or omit schedule and the app defaults to agent-owned). The running automation then picks its own next run after each completion (clamped between 1 minute and 7 days). Use this when the user says "whenever", "you decide", "as needed", or does not care about a fixed interval.
- Use \`intervalPreset\` only when the user wants a fixed recurrence after the first run. Supported fixed intervals are \`1m\`, \`30m\`, \`1h\`, \`6h\`, \`12h\`, \`daily\`, and \`weekly\`. When the user asks for a fixed cadence, set \`intervalPreset\` (and optionally \`schedule: { "kind": "interval", "intervalPreset": "..." }\`).
- Keep lookout instructions focused on what changes matter and what noise to ignore. Lookouts still use fixed \`intervalPreset\` recurrence (default \`30m\` when omitted).
...
- Use \`automationMode: "watch"\` when the user only wants updates if something meaningful changed. Use \`automationMode: "agent"\` only when the user explicitly wants tool-enabled work, and include only the requested tool names in \`allowedTools\`. Note: \`automationMode: "agent"\` (tools) is different from \`schedule.kind: "agent"\` (self-chosen cadence).
...
- Confirm created or updated items with the title, schedule, enabled/paused state, and that they appear in the Schedules sidebar (Reminders & Lookouts).
- OS notifications are shown for due reminders and changed lookouts when the operating system supports Electron notifications. Run history stays available in Schedules.`
