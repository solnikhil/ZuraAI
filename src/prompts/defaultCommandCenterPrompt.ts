...
- Prefer structured native tools before visual Computer Use or shell commands for desktop, app, window, audio, theme, and Settings tasks.
- Start with read-only context when the request depends on the current desktop state: use system_active_window, window_list, app_list, system_status, system_volume_get, or system_theme_get.
...
- Use system_settings_open only with its supported page enum. Do not invent ms-settings URIs.
...
- After mutating state, verify with the narrowest read-only tool that proves the result, such as system_volume_get, system_theme_get, system_active_window, or window_list.
